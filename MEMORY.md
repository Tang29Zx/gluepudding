...
- 2026-07-11：出生点视觉系统收口为“低多边形浮岛 × 暮紫占卜屋 × 青绿实验室”。初始视角固定为 `pitch=-0.01` / `yaw=0.05`，让占卜屋、中央石径和右侧实验室构成首帧三角层级。`WorldComposition.tsx` 使用现有 `TerrainSampler` 生成无碰撞的贴地石径、岩石、草簇、灯笼和五子棋方向标记；后续改位置时应继续采样地形，不要改成大块悬浮平面。
- 2026-07-11：户外可走地形顶点色在运行时以轻量方式降饱和，户外雾为 `58～126m`，Canvas 曝光为 `0.9`，阴影使用 `PCFSoftShadowMap`。`central-decor.glb` 包含大树，开启主光投影会在出生点形成大块黑影，因此中央装饰继续接收阴影但不投射阴影；占卜屋、实验室和近景程序几何体仍保留软阴影。
- 2026-07-11：2D 兜底页改用深墨紫 / 羊皮纸白 / 旧金视觉系统，真实世界主视觉为 `app/nav-world/public/images/world-overview.webp`。兜底页不再出现 Layer 编号、“未来接入”或“后续实现”文案；占卜屋、实验室和五子棋通过纵向编号列表提供总站、IoT 和五子棋独立降级入口。
- 2026-07-11：视觉收口版已发布到 `releases/20260711045314-91a5231`并切换 `current`。发布脚本原先在 `systemctl restart` 后立即单次请求健康端点，可在 Node 完成监听前误判失败并回滚；现改为每 0.5 秒一次、最长 20 秒短轮询，真实不健康时仍保留原子回滚。
- 2026-07-11：户外本地光影基线改为均衡晚午暖阳：程序化 `Sky`，`Environment` 只更新一帧且分辨率为 `64px`、强度 `0.38`，Canvas ACES 曝光为 `0.86`；暖色主光负责唯一一组阴影，冷色半球光和侧向光负责暗部可读性。背光夕阳方案会让建筑暗部过黑，后续不要在没有补充局部照明的情况下直接恢复。
- 2026-07-11：低角度太阳下，可走地形必须保持 `receiveShadow=true`、`castShadow=false`，否则低面数地表会出现明显 self-shadow acne；建筑和近景装饰仍可投影，`central-decor.glb` 继续沿用不投影约束。户外不引入 Bloom、SSAO、PCSS 或实时光追，灯笼仅使用无阴影短距离点光。
- 2026-07-11：暮光写实光影增强已按用户指令发布到 `releases/20260711060605-91a5231` 并切换 `current`。发布时前端 TypeScript / Vite 构建、fortune AI 11 项测试、生产依赖安装、systemd 内部健康、Nginx 配置与 reload 均通过；生产首页引用新版 `WorldExperience-DofON5dw.js`，公共 `/api/fortune/health` 返回成功。
- 2026-07-11：实验室真实 `robot001` 流为 1280×720 H.264 High Profile Level 3.1，MediaMTX WHEP / ICE 链路可正常建立。实验室大屏的媒体元素必须挂载在 DOM 视口外，不能使用完全 detached video；收到 WebRTC track 不等于画面可用，必须等 `readyState >= HAVE_CURRENT_DATA`、分辨率非零和首个视频帧后再创建 `VideoTexture`、显示在线。首帧超时需要进入指数退避重连，卸载时必须取消帧回调、清理 `srcObject`、移除媒体节点并释放纹理。
- 2026-07-11：实验室真实流首帧修复已发布到 `releases/20260711064416-e0237e5`，生产首页使用 `WorldExperience-CBhqJkaN.js`。发布后首页、主世界 JS、fortune health 为 `200`，未登录 `/lab-webrtc/robot001/whep` 继续返回 `401`；headless SwiftShader 不能作为 `VideoTexture` 最终肉眼验收，真实 GPU 浏览器仍需用户观察现场画面。
- 2026-07-11：Windows Edge 实机证明 WebRTC `VideoTexture` 即使已解码并进入在线状态仍可能为空，因此实验室大屏不再使用 WebGL 视频纹理；生产显示路径改为 drei `Html transform` 中的原生 video，720×405 CSS 基准按世界宽度计算 `distanceFactor`，保留 3D 边框、首帧门槛、在线角标和权限逻辑。WHEP 清理必须先 DELETE，再关闭 peer，或在 1.2 秒后兜底关闭，避免 peer close 与 DELETE 竞态产生 400。
- 2026-07-11：实验室原生视频合成修复已发布到 `releases/20260711071254-e0237e5`，生产主世界资源为 `WorldExperience-w23zXzKB.js`；首页、新版资源、fortune health 和在线 `robot001` 复检通过。断线恢复曾触发重复发布进程，最终只切换该 release；`scripts/deploy-production.sh` 现用 `/tmp/gluepudding-deploy-production.lock` 的非阻塞 `flock` 阻止并发发布。
- 2026-07-11：用户将樱花树 LOD 显示策略改为“质量只升不降”。后台队列仍按占卜屋室内、中模、高模执行；中模完成下载、解析和挂载后立即在全场替换低模，高模 ready 后立即替换中模，不再根据玩家与樱花树的距离降级。旧模型必须保留到新模型 ready，避免切换空白。
- 2026-07-11：樱花树渐进质量升级已从当前工作区发布到 `releases/20260711123738-cb69e27`；生产主世界资源为 `WorldExperience-BGT18mOB.js`，入口资源为 `index-C2XWOPqz.js`。发布脚本完成前端构建、fortune AI 11 项测试、两轮依赖审计、原子切换、服务健康检查和 Nginx reload；源站及公网首页均返回新哈希，未登录 WHEP 继续返回 `401`。
- 2026-07-11：部署脚本不要在 `npm ci` 后使用 `npx tsc` / `npx vite`；npm 11 曾在输出完成后短时间内尚未落盘 `.bin/tsc`，导致 `npx` 误提示安装无关的 `tsc@2.0.3`。脚本现在等待本地 binary 并直接调用 `./node_modules/.bin/tsc` 与 `./node_modules/.bin/vite`。
- 2026-07-11：实验室 Edge 空视频最终修复已发布到 `releases/20260711071254-e0237e5`，生产主世界资源为 `WorldExperience-w23zXzKB.js`。线上首页、JS、fortune health 为 `200`，WHEP 未登录为 `401`，`robot001` 在线；最终生产大屏走原生 HTML video 合成层，不再走 `VideoTexture`。
