...
## 2026-07-10 / 占卜屋入口雾与室内下载门槛

现象：右上角仍常驻显示 `Layer 5.5`，无法反映按需模型的真实下载状态；原 `InteriorReadyMarker` 位于资源 Suspense 之外，存在室内 GLB 尚未完成解析就放行入口雾的可能。

解决方案：右上角改为条件式模型下载 HUD，普通模型加载显示全局进度，占卜屋触发后优先显示独立的室内字节进度。室内 14 个 GLB 使用 4 路高优先级 fetch、浏览器缓存和自动重试；下载完成后再挂载隐藏室内场景预解析。Ready 标记移入同一个 Suspense，只有全部模型与室内功能组件提交后才允许全屏雾开始消散。

涉及文件：`startupAssetPreloader.ts`、`WorldExperience.tsx`、`FortuneAssetStage.tsx`、`styles.css`、`worldAssetManifest.ts`。

验证结果：仅运行 TypeScript 编译检查并通过；按用户要求未运行 Playwright，也未做截图或浏览器烟雾测试。

画面变化：模型加载期间右上角出现下载条；无加载时完全隐藏。占卜屋资源未就绪时，进入门口后会停留在全屏雾中。

补充修正：用户实机发现 Three.js 全局文件计数只能显示“正在下载模型 0%”，不包含具体资源和速度，并会在网络完成、解析尚未完成时消失。现新增 `TrackedModelGate`，把樱花树中/高 LOD 和五子棋也纳入与占卜屋一致的字节追踪；HUD 显示具体分组、当前文件、已下载/总大小和 KB/s 或 MB/s，解析阶段保持显示“本地解析中”。多任务用任务表保存，不会因为另一个文件完成而误删仍在运行的下载。
