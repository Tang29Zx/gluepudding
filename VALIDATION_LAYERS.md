...
- 2026-07-11：樱花树显示改为后台质量渐进升级；中模 ready 后立即替换低模，高模 ready 后立即替换中模，不再按距离降回低质量模型。切换前保留旧模型。TypeScript、Vite 构建和资源检查通过，生产已切换到 `releases/20260711123738-cb69e27`；当前共享环境的 headless SwiftShader 在 180 秒内未通过初始 3D 编译门槛，本次未生成桌面端 / 手机端截图，需在真实 GPU 浏览器复验高模完成后的画面与性能。
...
- 2026-07-09：空中实验室摆放、传送台、可踩表面、传送交互和玻璃边界碰撞的后续微调记录见本层 `debug.md`；模型 / 移动手感截图由用户实机验证，Codex 侧以资源检查和构建验证兜底。
- 2026-07-09：地面传送台 `Space` 上行新增 `admin` / `armbot` / `door` 任一角色权限门槛和世界内原生登录屏；交互细节和同源 auth 代理验证见本层 `debug.md`。
- 2026-07-09：占卜帐篷和地面传送台中间新增本地调试小屏，可临时切换实验室测试登录 / 未登录状态；上线前需替换为说明 / 关于屏。
- 2026-07-09：修复多次错误密码后登录屏隐藏但键盘捕获残留的问题；`Esc` 和点击框外作为父级兜底恢复移动，细节见本层 `debug.md`。
- 2026-07-09：隐藏出生点附近白粉色柱状比例参考人物，避免干扰正式场景视线。
...
- 2026-07-09：用户确认 Layer 8 验收通过；本层以星座、三张塔罗、周易六爻和周易抽签流程作为本次验收范围，`single` 塔罗模式和 78 张完整牌组保留为后续增强。
- 2026-07-10：占卜 AI 完成 P0 安全整改：独立服务、登录鉴权、普通 / Admin 分路由、普通用户限额与费用熔断、容量受控缓存、严格请求契约、Nginx 第一层保护及静态生产部署已落地。用户随后明确“不要测试”，完整 E2E 不再继续；详见 [Layer 8 debug.md](validation/layer-8/debug.md)。
...
- 2026-07-09：用户确认 Layer 9 待验证项已完成；本层以 MediaMTX WHEP 同源代理、`MediaStream -> VideoTexture` 大屏接入、mock / offline / unauthorized 降级状态作为完成口径。
...
- 2026-07-09：用户要求 Layer 10 标注搁置；真实 RDK 模型接入暂不作为当前验收阻塞项，后续恢复时需重新确认 RDK 数据来源、更新协议和展示台验收标准。
...
- 2026-07-09：用户要求 Layer 11 标注搁置；真实门禁控制暂不作为当前验收阻塞项。已有登录态和实验室进入权限只用于世界交互门槛，不等同于真实门禁接口授权。
...
- Layer 10 / Layer 11 已标注搁置，不作为本层阻塞项。
- Layer 6 世界内五子棋模拟层可用。
- 五子棋真实集成按 Layer 6 世界内五子棋基础独立推进，门禁权限和失败状态不作为本层依赖。
